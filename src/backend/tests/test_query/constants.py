...
COLLECTOR_SEARCH_DATA_RESP = {
    ...
    "query_sql": "SELECT * FROM test_rt WHERE `system_id` IN ('s1','s2') AND `action_id` IN ('create_link_table') "
    ...
    "count_sql": "SELECT COUNT(*) `count` FROM test_rt WHERE `system_id` IN ('s1','s2') AND `action_id` "
